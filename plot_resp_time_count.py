...
def plot_it():
    ...
    plt.ylabel('Response Count')
    """ax2=ax1.twinx()
    ax2.plot(timeframe,L8,'b-')
    #ax2.set_ylabel('Response Time')
    #plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,ncol=2, mode="expand", borderaxespad=0.)
    # red_patch = mpatches.Patch(color='red', label='transliterated')
    # ax1.legend(handles=[red_patch])
    # blue_patch = mpatches.Patch(color='blue', label='translated')
    # ax2.legend(handles=[blue_patch])
    # ax1.legend(handler_map={linetransli: HandlerLine2D(numpoints=4)})
    # ax2.legend(handler_map={linetranslat: HandlerLine2D(numpoints=4)})"""
